...
MODEL_NAME_sb = "blur_True_True_model"
MODEL_NAME_sc = "blur_True_False_model"
MODEL_NAME_rb = "blur_False_True_model"
MODEL_NAME_rc = "blur_False_False_model"


x_test,y_test,u_test,xnorm_test,ynorm_test,unorm_test,brel = gen_ede_deblur_data(
                                            freqmag,freqidx,test_siz,sig,blur_sig )
np.save(MODEL_SAVE_PATH+'x_test',x_test)
np.save(MODEL_SAVE_PATH+'y_test',y_test)
np.save(MODEL_SAVE_PATH+'u_test',u_test)
np.save(MODEL_SAVE_PATH+'xnorm_test',xnorm_test)
np.save(MODEL_SAVE_PATH+'ynorm_test',ynorm_test)
np.save(MODEL_SAVE_PATH+'unorm_test',unorm_test)


x_test = np.load(MODEL_SAVE_PATH+'x_test.npy')
y_test = np.load(MODEL_SAVE_PATH+'y_test.npy')
u_test = np.load(MODEL_SAVE_PATH+'u_test.npy')
xnorm_test = np.load(MODEL_SAVE_PATH+'xnorm_test.npy')
...
f_blur = np.reshape(u_test[:,::2],[input_size])
...
p1.plot(k,f_blur,'r',label='f_blur',linewidth=0.5)
...
p2.plot(k,f_blur,'r',label='f_blur',linewidth=1)
