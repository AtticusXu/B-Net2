...
class ButterflyLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, N, in_siz, out_siz, prep,
            channel_siz = 4, nlvl = -1, prefixed = False,
        ...
        self.k1lvl          = max(np.floor(
                                nlvl-np.log2(in_siz)).astype('int'), 0)
        self.k2lvl          = min(np.floor(
                                np.log2(out_siz)).astype('int'), nlvl)
        self.in_range       = in_range
        self.out_range      = out_range
        self.prep           = prep

        self.in_filter_siz  = max(1, in_siz // 2**nlvl)
        self.out_filter_siz = max(1, out_siz // 2**nlvl)
    ...
    def buildButterfly(self):
        ...
        ChebNodes = (np.cos(np.array(range(2*NG-1,0,-2))/2/NG*math.pi) \
                + 1)/2
        xNodes = np.arange(0,1,1/in_filter_siz)
        ...
        for it in range(0,NG):
            KVal = np.sin(math.pi * N \
                    * kcen * (xNodes - ChebNodes[it]) * xlen)
            LVec = np.squeeze(LMat[:,it])
            mat[:,0,2*it  ] =  np.multiply(KVal,LVec)
            mat[:,0,2*it+1] = -np.multiply(KVal,LVec)
        ...
        for lvl in range(1,k1lvl+1):
            ...
            for itk in range(0,2**lvl):
                ...
                for it in range(0,NG):
                    KVal = np.sin(math.pi * N * kcen \
                            * (x1Nodes-ChebNodes[it]) * xlen)
                    LVec = np.squeeze(LMat1[:,it])
                    mat[0,range(0,2*NG,2),2*it  ] = \
                            np.multiply(KVal,LVec)
                    mat[0,range(1,2*NG,2),2*it  ] = \
                            - np.multiply(KVal,LVec)
                    mat[0,:,2*it+1] = - mat[0,:,2*it]
        ...
        for lvl in range(k1lvl+1,k2lvl+1):
            ...
            for itk in range(0,2**lvl):
                ...
                for it in range(0,NG):
                    ...
                    mat[1,range(0,2*NG,2),2*it  ] = \
                            np.multiply(KVal,LVec)
                    mat[1,range(1,2*NG,2),2*it  ] = \
                            - np.multiply(KVal,LVec)
                    mat[1,:,2*it+1] = - mat[1,:,2*it]
        ...
        for itk in range(0,2**k2lvl):
            ...
            kNodes = np.arange(0,1,1/out_filter_siz)*klen + koff
